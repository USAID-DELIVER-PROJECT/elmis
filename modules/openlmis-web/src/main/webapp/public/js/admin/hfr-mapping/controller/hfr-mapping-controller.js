...
function HfrMappingController($scope,$route,GetHfrFacilityMappingList,TreeGeographicZoneListByProgram) {

    $scope.formMessage = 'header.interfaces.add.new';
    if($route.current.params.id)
        $scope.formMessage = 'header.interfaces.add.edit';

    GetHfrFacilityMappingList.get({}, function (data) {
       console.log(data);
    });

    TreeGeographicZoneListByProgram.get({
        program: parseInt(82,10)
    }, function (data) {
