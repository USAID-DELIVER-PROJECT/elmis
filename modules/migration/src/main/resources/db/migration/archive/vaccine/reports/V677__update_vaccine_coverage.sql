...
CREATE OR REPLACE VIEW vw_vaccine_coverage AS
 SELECT a.program_id,
   ...
   FROM ( WITH temp AS (
                       ...
                       products.primaryname AS display_name,
                      ...
                      case when  products.code = (select value from configuration_settings where key = 'VACCINE_DROPOUT_BCG') and doseid = 1 then (COALESCE(regularmale, 0) + COALESCE(regularfemale, 0)) else 0 end  bcg_1,
                      case when  products.code = (select value from configuration_settings where key = 'VACCINE_DROPOUT_MR') and doseid = 1 then (COALESCE(regularmale, 0) + COALESCE(regularfemale, 0)) else 0 end mr_1,
                      case when  products.code = (select value from configuration_settings where key = 'VACCINE_DROPOUT_DTP') and doseid = 1 then (COALESCE(regularmale, 0) + COALESCE(regularfemale, 0)) else 0 end dtp_1,
                      case when  products.code = (select value from configuration_settings where key = 'VACCINE_DROPOUT_DTP') and doseid = 3 then (COALESCE(regularmale, 0) + COALESCE(regularfemale, 0)) else 0 end dtp_3
         ...
         SELECT b.program_id,
            ...
            b.dtp_1,
            b.dtp_3
           FROM temp b) a order by a.display_order;
