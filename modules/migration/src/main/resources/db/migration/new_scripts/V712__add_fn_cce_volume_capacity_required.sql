...
net_volume=(Select sum(fic) as netvolumeperfic, sum(ficwithdiluent) as netvolumeperficwithdiluent from(
	...
	where pc.code=vaccine_category and pp.active=true) as fic_table);
...
net_volume_per_fic=net_volume.netvolumeperfic;
...
net_volume_per_fic=net_volume.netvolumeperficwithdiluent;
END IF;

volume_required=(((population * net_volume_per_fic*(coverage/100))/1000)/12)*max_months_of_stock;
