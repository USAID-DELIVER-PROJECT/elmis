DROP FUNCTION IF EXISTS fn_vaccine_geozone_n_rnrs(character varying, integer, integer, character varying, integer, integer);

CREATE OR REPLACE FUNCTION fn_vaccine_geozone_n_rnrs(IN in_program_code character varying, IN in_period_id integer, IN in_geographiczone_id integer, IN in_product_code character varying, IN in_n integer DEFAULT 4, IN in_userid integer DEFAULT 0)
...
price
from fn_vaccine_geozone_nth_rnr('||t_program_id||','||t_period_id||','||in_geographiczone_id||','||t_product_id||','||i||','||in_userid||')';
i = i+1;
end loop;
