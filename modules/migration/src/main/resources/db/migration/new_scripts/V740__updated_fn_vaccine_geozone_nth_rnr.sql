DROP FUNCTION IF EXISTS fn_vaccine_geozone_nth_rnr(integer, integer, integer, integer, integer, integer);

CREATE OR REPLACE FUNCTION fn_vaccine_geozone_nth_rnr(IN in_program_id integer, IN in_period_id integer, IN in_geographiczone_id integer, IN in_product_id integer, IN in_nth integer DEFAULT 0, IN in_userid integer DEFAULT 0)
...
if in_userid > 0 THEN
t_where_1 = t_where_1 || ' and facilities.geographiczoneid in (select district_id from vw_user_districts where user_id = '||in_userid ||')';
end if;
...
sum(closingbalance)::int endingbalance,
