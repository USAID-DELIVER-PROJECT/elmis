DROP MATERIALIZED VIEW IF EXISTS vw_vaccine_inventory_stock_status_dashboard ;

CREATE MATERIALIZED VIEW vw_vaccine_inventory_stock_status_dashboard AS
 ...
 f.id AS facility_id,
   ...
   FROM stock_cards sc
     ...
     LEFT JOIN program_products pp ON pp.programid = (( SELECT fn_get_vaccine_program_id() AS fn_get_vaccine_program_id)) AND pp.productid = sc.productid
