 ...
 WITH t AS (
         SELECT geographic_zones.id AS geographic_zone_id,
           ...
           FROM vaccine_report_logistics_line_items
             ...
             JOIN products ON products.id = program_products.productid
          ...
          WHERE program_products.active = true AND processing_periods.numberofmonths = 1 and  ft.levelid=( select max(levelid) from facility_types )
        )
